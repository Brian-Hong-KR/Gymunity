...
public class PointServiceImp implements PointService{
	...
	public void rewardProcess(PointDTO dto) {	
		...
		int bettingPoint = pointRepository.findTotalPoint(memDTO.getMem_user_id(), memDTO.getMem_ch_id()); // 배팅포인트
		

		double archiveRate = pointRepository.findRate(memDTO.getMem_user_id(), memDTO.getMem_ch_id()); // 성공률
		...
		int rewardPoints;
		 ...
		 if (archiveRate >= 100) {
		        ...
		        // 60% 포인트 부여
		        rewardPoints = (int) (bettingPoint  * 0.6);
		    } else if (archiveRate >= 50) {
		        // 30% 포인트 부여
		        rewardPoints = (int) (bettingPoint  * 0.3);
		    } else {
		        // 보상 없음
		        rewardPoints = 0;
		    }
