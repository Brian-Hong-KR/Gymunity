...
const UserManagePage = () => {
  ...
  const handleGetUserDetails = () => {
    axios
      .get(`${gConst.API_BASE_URL}:8090/getUserDetails/${nickName}`)
      .then((response) => {
  ...
  const handleToggleIsActive = () => {
    ...
    axios
      .put(`${gConst.API_BASE_URL}:8090/updateIsActive?userId=${userId}`)
      .then(() => {
  ...
  const handleCheckAndUpdateNickName = async () => {
    ...
    try {
      const response = await axios.get(
        `${gConst.API_BASE_URL}:8090/checkUserNickname/${newNickName}`
      );
      ...
      if (response.status === 200) {
        ...
        if (userInfo) {
          ...
          axios
            .put(
              `${gConst.API_BASE_URL}:8090/updateNickName?userId=${userId}&nickName=${newNickName}`
            )
            .then(() => {
              setUserInfo((prevUserInfo) => ({
                ...prevUserInfo,
